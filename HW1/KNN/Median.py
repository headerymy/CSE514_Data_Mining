...
'''
use numpy package to calculate the median value of each columns
'''
def median(lst):
        ...
        for col in range(len(data[row])):
            ...
            try:
                ...
                if col in mediandict.keys():
                    median_value = mediandict.get(col)
                else:
                    #store the value that is not inside the dictionary into the dictionary
                    median_value = calmedianvalue(data, col)
                    mediandict[col] = median_value
                #add median value
...
if __name__ == '__main__':
    ...
    #replace NA by median value
    print('2.Use median value to replace NA')
    n_data1 = replacenabymedian(data1)
    ...
    print('4.Use median value to replace the random NA')
    n_data2 = replacenabymedian(data2)
